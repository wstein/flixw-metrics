...
/** Exercises the pre-0.68 adapter against its oldest supported real compiler. */
public final class Flix0672AdapterTest {
    private Flix0672AdapterTest() { }

    public static void main(String[] args) throws Exception {
        Path project = Flix0753AdapterTest.copyFixture(Path.of(args[0]));
        try {
            Model model = new Flix0672Adapter().measure(project);
            ...
            require(definition(model, "testSelectValue").effects()
                    .equals(java.util.List.of("Assert", "IO")),
                "the adapter preserves the older compiler's normalized declared effects");
            ...
            Flix0753AdapterTest.assertRestrictableEnums(
                new Flix0672Adapter().measure(project));
            ...
            Flix0753AdapterTest.assertEffectMetrics(new Flix0672Adapter().measure(project));
            Flix0753AdapterTest.writeHandlerFixture(project);
            Flix0753AdapterTest.assertHandlerMetrics(new Flix0672Adapter().measure(project));
            System.out.println("Flix0672AdapterTest: ok");
        } finally {
