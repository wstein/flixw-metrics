...
Flix **0.68.0, 0.75.2, 0.75.3, and 0.76.0** are verified. Flix 0.68.0 and 0.75.2 use
`Flix0680Adapter`; Flix 0.75.3 and 0.76.0 use `Flix0753Adapter`. Each name records the oldest
...
The full test suite also locks the lower boundary: it requires 0.67.0 to fail the derived ABI gate
and runs the packaged integration test with 0.68.0, 0.75.2, and 0.75.3. A release not named above
may still link, but it is unsupported until its own compatibility investigation is recorded.
