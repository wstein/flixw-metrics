...
`adapter0680/src/` and `adapter0672/src/`. The stable boundary is under
...
`flix0672/Flix0672Adapter.scala`. Everything else — `Main`, `Metrics`, `ResultCache`, `SourceMetrics`, and
...
`Flix0672Adapter` is verified with Flix 0.67.2; `Flix0680Adapter` with Flix 0.68.0 and 0.75.2;
and `Flix0753Adapter` with Flix 0.75.3 and 0.76.0. Each name records the oldest verified release
in its linkage family.
...
- `make test` runs linting, calibration and performance-contract tests, all executable unit tests,
  the real-compiler fixture, integration tests, and the 0.67.1/0.67.2 compatibility boundary. It
  also verifies packaged operation on 0.68.0, 0.75.2, and 0.75.3. Historical compiler jars are downloaded
