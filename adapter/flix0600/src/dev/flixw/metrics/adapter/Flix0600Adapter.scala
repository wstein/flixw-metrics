 ...
 *
 * `Flix0600` names the oldest verified release in this linkage family, not its newest supported release.
 * Typed Scala patterns make misspelled or removed
 ...
 */
final class Flix0600Adapter extends CompilerModel {
  ...
  private def measureSelected(projectRoot: Path, include: Source => Boolean): Model = {
    ...
    val bootstrap = Bootstrap.bootstrap(projectRoot, None)(Formatter.getDefault, System.err) match {
      case ca.uwaterloo.flix.util.Validation.Success(b) => b
      case ca.uwaterloo.flix.util.Validation.Failure(e) =>
  ...
  private def measureEffect(effect: TypedAst.Effect, projectRoot: Path): EffectInfo =
    new EffectInfo(effect.sym.toString, relativise(effect.loc, projectRoot), effect.loc.beginLine,
      0, effect.ops.map(op => new EffectOperationInfo(op.sym.name,
    ...
    tokens.foreach { t =>
      if (t.kind != ca.uwaterloo.flix.language.ast.TokenKind.Eof) {
        if (!t.kind.isComment) {
          ...
          tokenCounts.update(t.sp1.lineOneIndexed,
            tokenCounts.getOrElse(t.sp1.lineOneIndexed, 0) + 1)
        }
        val target =
          if (!t.kind.isComment) codeLines
          else if (t.text.startsWith("///")) docLines
          else commentLines
        for (line <- t.sp1.lineOneIndexed to t.sp2.lineOneIndexed) target += line
      }
    }
  ...
  private def projectSource(src: Source, projectRoot: Path): Boolean = src.input match {
    case Input.TxtFile(path, _) => path.toAbsolutePath.normalize.startsWith(projectRoot)
    case _ => false
  }

  private def sourceName(src: Source, projectRoot: Path): String = src.input match {
    case Input.TxtFile(path, _) => projectRoot.relativize(path.toAbsolutePath.normalize)
      .toString.replace('\\', '/')
    case Input.Text(name, _, _) => name.replace('\\', '/')
    case _ => src.name.replace('\\', '/')
  }

  private def compilerSource(src: Source, virtualPath: String): Boolean = src.input match {
    case Input.Text(name, _, _) => name.replace('\\', '/') == virtualPath
    case _ => false
  }
  ...
  private def measureDef(d: TypedAst.Def, projectRoot: Path,
    ...
    val codeLines = (d.loc.beginLine to d.loc.endLine).count(onLines.contains)
    // The densest line this definition spans, and who actually owns it.
    val (crammedLine, crammedTokens) =
      (d.loc.beginLine to d.loc.endLine)
    ...
    node match {
      ...
      case e: TypedAst.Expr.LocalDef =>
        ...
        if (parameters > tally.widestLocalParams) {
          ...
          tally.widestLocalLine = e.loc.beginLine
        }
