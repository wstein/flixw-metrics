 ...
 */
final class Flix0761Adapter extends CompilerModel {
  ...
  private def projectSource(src: Source, projectRoot: Path): Boolean =
    src.origin.isUser && sourcePath(src).exists(_.startsWith(normalized(projectRoot)))

  private def sourceName(src: Source, projectRoot: Path): String =
    sourcePath(src).filter(_.startsWith(normalized(projectRoot)))
      .map(normalized(projectRoot).relativize(_).toString.replace('\\', '/'))
      .getOrElse(src.sourceName.toString.replace('\\', '/'))

  private def compilerSource(src: Source, virtualPath: String): Boolean =
    src.origin == Origin.Library &&
      src.sourceName.toString.replace('\\', '/') == virtualPath

  private def sourcePath(src: Source): Option[Path] =
    src.sourceName.toPath.map(normalized(_))
  ...
  private def projectFile(loc: SourceLocation, projectRoot: Path): Option[Path] =
    if (!loc.source.origin.isUser) None
    else sourcePath(loc.source).filter(_.startsWith(normalized(projectRoot)))
}
